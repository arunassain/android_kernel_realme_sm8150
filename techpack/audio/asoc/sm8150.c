...
static bool msm_usbc_swap_gnd_mic(struct snd_soc_codec *codec, bool active)
{
	...
	#else
	if(FSA4480 == wcd_mbhc_cfg.switch_type) {
		if (!pdata->fsa_handle)
			return false;

		ret = fsa4480_switch_event(pdata->fsa_handle, FSA_MIC_GND_SWAP);
		return (0 == ret);
	} else if(MAX20328 == wcd_mbhc_cfg.switch_type) {
		if (!pdata->max_handle)
			return false;
...
#define S(X, Y) ((WCD_MBHC_CAL_PLUG_TYPE_PTR(wcd_mbhc_cal)->X) = (Y))
	#ifndef VENDOR_EDIT
	/* Zhao.Pan@PSW.MM.AudioDriver.HeadsetDet, 2018/12/05,
...
#ifndef VENDOR_EDIT
/*Le.Li@PSW.MM.AudioDriver.SmartPA, 2018/09/23, Modify for Max98937*/
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	u32 val = 0;
	#ifdef VENDOR_EDIT
	/*Le.Li@PSW.MM.AudioDriver.SmartPA, 2018/09/23, Modify for Max98928*/
	int i;
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if(FSA4480 == wcd_mbhc_cfg.switch_type) {
		...
		pr_err("%s: unsupport usbc switch, value = %d, \n", __func__, wcd_mbhc_cfg.switch_type);
	}
	#endif /* VENDOR_EDIT */
	...
	#ifdef VENDOR_EDIT
	/*Le.Li@PSW.MM.AudioDriver.Machine, 2018/09/23, Add for log*/
